...
@pytest.mark.skip(reason="Integration tests provide better coverage of CLI functionality")
def test_get_scale_command_success(runner, scale_app):
    ...
    with patch('tem_analysis_pipeline.cmd.scale_estimation.ScaleManager') as MockManager:
        # Configure the mock
...
def test_get_scale_command_no_scale_found(runner, scale_app):
    ...
    with patch('tem_analysis_pipeline.cmd.scale_estimation.ScaleManager') as MockManager:
        ...
        manager_instance = MockManager.return_value
        manager_instance.get_scale.side_effect = NoScaleFoundError("Couldn't find scale bar")
...
def test_get_scale_command_with_fallback(runner, scale_app):
    ...
    with patch('tem_analysis_pipeline.cmd.scale_estimation.ScaleManager') as MockManager:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = runner.invoke(scale_app, [str(test_img), "--fallback", str(fallback_scale)])
...
def test_get_scale_command_skip_missing(runner, scale_app):
    ...
    with patch('tem_analysis_pipeline.cmd.scale_estimation.ScaleManager') as MockManager:
        ...
        manager_instance = MockManager.return_value
        manager_instance.get_scale.side_effect = ScaleEstimationError("No suitable estimator found")
...
def test_get_scale_command_debug_mode(runner, scale_app):
    ...
    with patch('tem_analysis_pipeline.cmd.scale_estimation.ScaleBarReader') as MockReader:
    ...
    with patch('tem_analysis_pipeline.cmd.scale_estimation.ScaleManager') as MockManager:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('builtins.open', mock_open()) as mock_file:
                ...
                result = runner.invoke(scale_app, [
                    str(test_img),
