...
def test_scale_manager_fallback():
    ...
    assert result == fallback_scale, f"Expected fallback scale {fallback_scale}, got {result}"
...
def test_scale_manager_multiple_estimators():
    ...
    try:
        ...
        assert 0.0001 <= scale <= 100, f"Scale value {scale} outside expected range"
        print(f"Successfully estimated scale using fallback estimator: {scale} μm/pixel")
...
def test_process_batch_of_images():
    ...
    assert any(data["success"] for data in results.values()), "No images were successfully processed"
