...
def read_tif_image(image_path, resize_scale=1.0, gamma_correction_peak_target=None):
...
def read_png_image(image_path, resize_scale=1.0, smooth_radius=2):
...
def get_tiles_from_array(
    ...
    coord_candidates = [
        (i * strd_height, j * strd_width) for i in range(n_rows) for j in range(n_cols)
    ]

    # 2. Crop out the parts of image not fitting into tiles
    ver_border, hor_border = (
        (tile_height - wndw_height) // 2,
        (tile_width - wndw_width) // 2,
    )
    height_pad, width_pad = (
        (n_rows - 1) * strd_height + wndw_height - height,
        (n_cols - 1) * strd_width + wndw_width - width,
    )

    extended_img_array = np.pad(
        img_array,
        [[ver_border, ver_border + height_pad], [hor_border, hor_border + width_pad]],
        pad_mode,
    )
    ...
    for v_pos, h_pos in coord_candidates:
        img_tile = extended_img_array[
            v_pos : v_pos + tile_height, h_pos : h_pos + tile_width
        ]
...
def get_array_from_tiles(tiled_data, coord_list, window_shape=None, output_shape=None):
    ...
    for i, (ver_pos, hor_pos) in enumerate(coord_list):
        output_array[
            ver_pos : ver_pos + tile_height, hor_pos : hor_pos + tile_width
        ] += tiled_data[i]
        data_count[ver_pos : ver_pos + tile_height, hor_pos : hor_pos + tile_width] += 1

    output_array[output_array > 0] = (
        output_array[output_array > 0] / data_count[output_array > 0]
    )
    ...
    if output_shape is not None:
        if output_array.shape[0] >= output_shape[0]:
            ...
            output_array = np.pad(
                output_array, [[0, output_shape[0] - output_array.shape[0]], [0, 0]]
            )
        ...
        if output_array.shape[1] >= output_shape[1]:
            ...
            output_array = np.pad(
                output_array, [[0, 0], [0, output_shape[1] - output_array.shape[1]]]
            )
...
def write_tfrecord_for_image_mask_pair(
    image_path: str,
    tile_shape: Tuple[int, int],
    layer_depth: int,
    resize_scale: float = 1.0,
    gamma_correction_peak_target: float = 0.45,
    smooth_radius: int = 2,
    ...
    **kwargs,
    ...
    img_array = read_tif_image(
        image_path,
        resize_scale=resize_scale,
        gamma_correction_peak_target=gamma_correction_peak_target,
    )
    msk_array = read_png_image(
        label_path, resize_scale=resize_scale, smooth_radius=smooth_radius
    )

    # this is the size of the window at the output layer
    window_shape = (
        compute_output_size(input_size=tile_shape[0], layer_depth=layer_depth),
    ...
    image = (
        get_tiles_from_array(
            img_array, tile_shape, stride_shape, window_shape, pad_mode=pad_mode
        )
        / 255
    )
    label = (
        get_tiles_from_array(
            msk_array, tile_shape, stride_shape, window_shape, pad_mode=pad_mode
        )
        / 255
    )
...
def keep_fraction_of_empty_labels(fraction):
    def filter_function(image, label):
        ...
        return (
            tf.math.reduce_any(label > 0)
            or tf.math.reduce_sum(image) / 100 % 1 < fraction
        )
...
def random_image_adjust(image, label):
    ...
    label = tf.image.resize(
        label, [new_height, new_width], method="nearest", preserve_aspect_ratio=True
    )
