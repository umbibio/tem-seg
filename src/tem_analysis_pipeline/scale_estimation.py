...
class ScaleEstimator(ABC):
    ...
    def _convert_to_pil(self, image_input: Union[Path, Image.Image, np.ndarray]) -> Image.Image:
...
class ScaleBarReader(ScaleEstimator):
    ...
    def estimate_scale(self, image_input: Union[Path, Image.Image, np.ndarray]) -> float:
        ...
        """
        from .calibration import get_calibration, NoScaleError, NoScaleNumberError, NoScaleUnitError
        ...
        try:
            ...
            raise ScaleDetectionFailureError(f"Unexpected error in scale detection: {e}")
