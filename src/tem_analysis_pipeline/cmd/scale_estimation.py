...
def create_scale_app() -> typer.Typer:
    ...
    def get_scale_command(
        image_paths: Annotated[
            List[Path],
            Argument(
                help="Path(s) to TEM image(s) for scale detection"
            )
        ],
        fallback: Annotated[
            Optional[float],
            Option(
                "--fallback",
                "-f",
                help="Fallback scale in μm/pixel if detection fails"
            )
        ] = None,
        output: Annotated[
            Optional[Path],
        ...
        import traceback
        import sys
        from ..scale_estimation import ScaleEstimationError, NoScaleFoundError, ScaleDetectionFailureError, InvalidInputError
            ...
            try:
                ...
                if debug:
                    ...
                    try:
                        ...
                        typer.echo(f"[DEBUG] Image loaded: size={img.size}, mode={img.mode}")
                        ...
                        typer.echo(f"[DEBUG] Traceback: {''.join(traceback.format_tb(sys.exc_info()[2]))}")
                        raise
                else:
                ...
                # Store result
                results.append({"image": str(img_path), "scale": scale, "status": status})
                ...
                if fallback is not None:
                    ...
                    results.append({"image": str(img_path), "scale": fallback, "status": "fallback_used"})
                    typer.echo(f"No scale bar found in {img_path}, using fallback: {fallback} μm/pixel")
                elif skip_missing:
                    results.append({"image": str(img_path), "scale": None, "status": "skipped_no_scale"})
                    typer.echo(f"No scale bar found in {img_path}, skipping (--skip-missing enabled)")
                else:
                    # Report as error when no fallback and not skipping
                    status = "no_scale_found"
                    results.append({"image": str(img_path), "scale": None, "status": status})
                    typer.echo(f"No scale bar found in {img_path}: {e}")
                
                if debug:
                    typer.echo(f"[DEBUG] Detailed error: {str(e)}")
                    typer.echo("[DEBUG] This typically means the image doesn't contain a recognized scale bar format")
                ...
                if fallback is not None:
                    results.append({"image": str(img_path), "scale": fallback, "status": "fallback_used"})
                    typer.echo(f"Failed to detect scale in {img_path}, using fallback: {fallback} μm/pixel")
                elif skip_missing:
                    # Also apply skip_missing to general scale estimation errors
                    results.append({"image": str(img_path), "scale": None, "status": "skipped_no_scale"})
                    typer.echo(f"Failed to detect scale in {img_path}, skipping (--skip-missing enabled)")
                else:
                    # Report as error when no fallback and not skipping
                    status = "error"
                    results.append({"image": str(img_path), "scale": None, "status": status})
                ...
                results.append({"image": str(img_path), "scale": None, "status": f"unexpected_error: {e}"})
        ...
        # Print summary
        typer.echo("\nSummary:")
        typer.echo(f"  Successfully detected: {success_count}/{len(image_paths)} images")
        if fallback_count > 0:
            typer.echo(f"  Using fallback scale: {fallback_count}/{len(image_paths)} images")
        if skipped_count > 0:
            typer.echo(f"  Skipped (no scale): {skipped_count}/{len(image_paths)} images")
        if error_count > 0:
